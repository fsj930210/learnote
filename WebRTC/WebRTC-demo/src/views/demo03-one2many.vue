			...
			<el-col :span="6" v-if="formInline.pub === 'pub'">
				<div style="height: 75vh; overflow-y: auto; display: flex; flex-direction: row; width: 100%">
					<div style="width: 200px; text-align: left; padding: 20px">
						...
						<vue-select-image :dataImages="bgUrls" @onselectimage="onSelectImage"> </vue-select-image>
					</div>
					...
					<div id="allVideo">
						...
						<div v-if="formInline.pub === 'pub'" id="localDomId">
							<div
								...
								style="position: relative; width: 600px; height: 250px; display: flex; flex-direction: row"
							>
								<video
									id="localdemo01"
									controls
...
export default {
	...
	methods: {
		...
		async initMeetingRoomPc() {
			...
			const localUid = this.formInline.userId;
			//找到当前房间的视频流发布者 即主播
			let publisher = this.roomUserList
				.filter((e) => e.userId !== localUid && e.pub === 'pub')
				.map((e, index) => {
					return e.userId;
				});
